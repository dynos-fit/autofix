"""The ``autofix-next export-sarif`` subcommand (ACs 1-7, 14, 15, 25).
...
:func:`autofix_next.telemetry.sarif.emit_sarif`.
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser:
        ...
        :mod:`autofix_next.cli.main`. Mutated in place, matching the
        shape used by :func:`autofix_next.cli.replay_command.add_arguments`.
    """
    parser.add_argument(
        "--scan-id",
        required=True,
...
def run(args: argparse.Namespace) -> int:
    ...
    * otherwise                           -> 0 (AC 3)
    ...
    if not events_path.is_file():
        ...
        print(
            f"autofix-next export-sarif: events.jsonl not found at {events_path}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        print(
            f"autofix-next export-sarif: error reading {events_path}: {exc}",
            file=sys.stderr,
        )
        return 2
    ...
    if not anchor_found:
        ...
        print(
            f"autofix-next export-sarif: scan_id {scan_id!r} not found in {events_path}",
            file=sys.stderr,
        )
        return 1
    ...
    if args.out is None:
        try:
            ...
            print(
                f"autofix-next export-sarif: could not create temp file: {exc}",
                file=sys.stderr,
            )
            return 2
        try:
            ...
            print(
                f"autofix-next export-sarif: emit failed: {exc}",
                file=sys.stderr,
            )
            return 2
            ...
            print(
                f"autofix-next export-sarif: SARIF shape check failed: {exc}",
                file=sys.stderr,
            )
            return 3
        finally:

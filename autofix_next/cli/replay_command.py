"""The ``autofix-next replay`` subcommand (ACs 46-49).

Replays a past scan identified by ``--scan-id`` by delegating to
:func:`autofix_next.telemetry.replay.replay`. The ``replay`` engine parses
...
from autofix_next.telemetry.replay import ScanRun, replay
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser:
        ...
        in :mod:`autofix_next.cli.main`. This function mutates it in
        place, matching the shape used by
        :func:`autofix_next.cli.scan_command.add_arguments`.
    """
    parser.add_argument(
        "--scan-id",
        required=True,
...
def run(args: argparse.Namespace) -> int:
    ...
    * ``ValueError`` / ``FileNotFoundError`` from :func:`replay`
    ...
    branch is defensive; :mod:`autofix_next.telemetry.replay` only
    ...
    try:
        ...
        print(f"autofix-next replay: {exc}", file=sys.stderr)
        return 2
    ...
    if args.json:
        ...
        try:
            ...
            print(
                f"autofix-next replay: failed to serialize ScanRun: {exc}",
                file=sys.stderr,
            )
            return 2
        print(serialized)

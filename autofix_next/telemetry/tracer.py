"""OpenTelemetry tracer lifecycle for autofix_next (AC 1-5, 61)."""
...
def get_tracer() -> Tracer:
    ...
    with _LOCK:
        ...
        _TRACER = trace.get_tracer("autofix_next")
        return _TRACER
...
def reset_tracer_provider() -> None:
    """Test-only: clear the cached tracer. Requires AUTOFIX_NEXT_TESTING=1."""
    if os.environ.get("AUTOFIX_NEXT_TESTING") != "1":
        raise RuntimeError(
            "reset_tracer_provider() requires AUTOFIX_NEXT_TESTING=1 env var; "

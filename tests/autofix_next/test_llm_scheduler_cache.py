...
def _build(
    ...
    *,
    ...
    if use_env:
        ...
        monkeypatch.setenv("AUTOFIX_NEXT_LLM_CACHE_DIR", str(resolved))
    else:
        monkeypatch.delenv("AUTOFIX_NEXT_LLM_CACHE_DIR", raising=False)
...
def test_env_var_overrides_policy_cache_dir(tmp_path, monkeypatch):
    """AC 36: ``AUTOFIX_NEXT_LLM_CACHE_DIR`` takes precedence over policy key."""
    env_cache = tmp_path / "env_cache"
    policy_cache = tmp_path / "policy_cache"
    monkeypatch.setenv("AUTOFIX_NEXT_LLM_CACHE_DIR", str(env_cache))
    _write_policy(
        tmp_path,

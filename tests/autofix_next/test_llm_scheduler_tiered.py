"""Tests for the tiered routing surface of autofix_next.llm.scheduler.Scheduler.
...
def _build_scheduler(
    ...
    *,
    ...
    monkeypatch.setenv("AUTOFIX_NEXT_LLM_CACHE_DIR", str(cache_root))
...
def test_priority_high_uses_large_tier_template(tmp_path, monkeypatch):
    ...
    # against ``autofix_next/llm/prompts/``; we use the canonical on-disk
    # name for the large-tier rule.
    large_template = (
        REPO_ROOT / "autofix_next" / "llm" / "prompts"

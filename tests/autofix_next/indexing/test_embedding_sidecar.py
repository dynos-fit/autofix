"""Unit tests for the :mod:`autofix_next.indexing.embedding` sidecar.
...
from autofix_next.indexing.embedding import (
...
def test_public_surface_is_exactly_five_names() -> None:
    import autofix_next.indexing.embedding as mod
...
def test_disabled_methods_are_noops(tmp_path: Path) -> None:
    ...
    assert not (tmp_path / ".autofix-next" / "indexing").exists()
...
def test_save_load_roundtrip(tmp_path: Path) -> None:
    ...
    idx_path = tmp_path / ".autofix-next" / "indexing" / "embedding.hnswlib.idx"
    meta_path = (
        tmp_path / ".autofix-next" / "indexing" / "embedding-metadata.json"
    )

...
Covers:
  ...
  AC 7 — ``reset_adapter_emission_state`` raises ``RuntimeError`` without
         ``AUTOFIX_NEXT_TESTING=1`` and succeeds with it.
...
def _mod():
    return pytest.importorskip("autofix_next.languages.adapter_emission")
...
# ---------------------------------------------------------------------------
# AC 7 — reset guard on AUTOFIX_NEXT_TESTING.
# ---------------------------------------------------------------------------
...
def test_reset_requires_testing_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``AUTOFIX_NEXT_TESTING`` is unset (or not exactly ``"1"``)."""
    ae = _mod()

    monkeypatch.delenv("AUTOFIX_NEXT_TESTING", raising=False)
    with pytest.raises(RuntimeError) as excinfo:
        ae.reset_adapter_emission_state()
    assert "AUTOFIX_NEXT_TESTING" in str(excinfo.value)

    # Wrong value also fails — not just any non-empty string.
    monkeypatch.setenv("AUTOFIX_NEXT_TESTING", "0")
    with pytest.raises(RuntimeError):
        ae.reset_adapter_emission_state()

    monkeypatch.setenv("AUTOFIX_NEXT_TESTING", "true")
    with pytest.raises(RuntimeError):
        ae.reset_adapter_emission_state()
...
def test_reset_succeeds_under_testing_env_and_clears_state(
    ...
    """AC 7: with ``AUTOFIX_NEXT_TESTING=1``, ``reset_adapter_emission_state``

...
def test_benchmark_adapter_imports_resolve() -> None:
    ...
    # The adapter must import from the new namespace, not the legacy autofix_next.
    assert ("autofix" + "_next") not in text, (
        "benchmark adapter still references autofix_next"
    )
...
def test_scheduler_imports_llm_seam_same_package() -> None:
    """AC 16: scheduler.py imports llm_backend without crossing into autofix_next."""
    from pathlib import Path
    ...
    assert ("autofix" + "_next") not in text, (
        "scheduler.py still references autofix_next"
    )

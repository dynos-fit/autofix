"""Tests for ``autofix_next.llm.triage`` and ``autofix_next.llm.report_writer``.
...
task-20260417-008:
  ...
  AC 29 — ``autofix_next/llm/prompts/unused-import_intra-file__large.md``
...
TRIAGE_PATH = REPO_ROOT / "autofix_next" / "llm" / "triage.py"
REPORT_PATH = REPO_ROOT / "autofix_next" / "llm" / "report_writer.py"
PROMPTS_DIR = REPO_ROOT / "autofix_next" / "llm" / "prompts"
...
def test_resolve_template_signature_and_returns_path():
    ...
    from autofix_next.llm.triage import resolve_template
...
def test_report_writer_write_signature_and_returns_record():
    ...
    from autofix_next.llm.report_writer import ReportRecord, write

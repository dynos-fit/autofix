"""Smoke tests for all 5 autofix-next subcommands invoked via main([...]).

Covers AC 20-22 (spec §20, §21, §22):

- AC 20: five smoke tests covering scan, watch, replay, export-sarif,
         policy --show; each invokes autofix_next.cli.main.main([...])
...
autofix_next/events/schema.py ScanEvent fields before fixture authoring.
...
def test_scan_runnable(
    ...
    """AC 20: autofix-next scan exits 0 with run_scan stubbed out.

    Uses monkeypatch.chdir to ensure --root resolves to the fixture repo.
    """
    from autofix_next.funnel.pipeline import ScanResult
    ...
    with patch(
        ...
        with patch("autofix_next.telemetry.sarif.emit_sarif"):
...
def test_watch_runnable(
    ...
    """AC 20: autofix-next watch exits 0 with AUTOFIX_NEXT_WATCH_ONCE=1
    ...
    monkeypatch.setenv("AUTOFIX_NEXT_WATCH_ONCE", "1")
    ...
    with patch(
        "autofix_next.cli.watch_command.WatcherSession",
...
def test_replay_runnable(
    ...
    """AC 20: autofix-next replay exits 0 with replay() stubbed to return
    a 'match' ScanRun so version-drift does not affect the test."""
    from autofix_next.telemetry.replay import ScanRun
...
def test_export_sarif_runnable(
    ...
    """AC 20: autofix-next export-sarif exits 0 with emit_sarif stubbed and
    a seeded events.jsonl containing the ScanStarted anchor."""
    out_path = tmp_path / "smoke.sarif"

    with patch(
        "autofix_next.cli.export_sarif_command.emit_sarif"
    ) as mock_emit:
...
def test_policy_show_runnable(
    ...
    """AC 20: autofix-next policy --show exits 0 and stdout contains 'llm_tiered'."""
    from autofix_next.cli.main import main

"""Tests for autofix_next/cli/watch_command.py (NEW production module).
...
- AC 6: AUTOFIX_NEXT_WATCH_ONCE=1 env causes the watcher to process 1 event
        then exit 0
- AC 7: is_fresh_instance from Watchman propagates to ChangeSet (mock
        WatcherSession.next_files)

These tests MUST FAIL until production code at autofix_next/cli/watch_command.py
lands.
"""
...
class TestWatchAddArguments:
    ...
    def test_root_flag_registered(self) -> None:
        ...
        from autofix_next.cli.watch_command import add_arguments
...
class TestWatchDiagnosticWhenPywatchmanMissing:
    ...
    def test_missing_pywatchman_returns_exit_2(
        ...
        from autofix_next.cli.watch_command import add_arguments, run
...
# ---------------------------------------------------------------------------
# AC 6: AUTOFIX_NEXT_WATCH_ONCE=1 escape hatch
# ---------------------------------------------------------------------------


class TestWatchOnceEscapeHatch:
    """AC 6 — AUTOFIX_NEXT_WATCH_ONCE=1 causes the watcher to process exactly
    ...
    def test_watch_once_env_exits_zero(
        ...
        """AC 6: AUTOFIX_NEXT_WATCH_ONCE=1 -> exit 0."""
        ...
        with patch(
            ...
            with patch("autofix_next.funnel.pipeline.run_scan") as mock_run_scan:
    ...
    def test_watch_once_truthy_variants_exit_zero(
        ...
        AUTOFIX_NEXT_WATCH_ONCE — use 'true' as an example."""
        monkeypatch.setenv("AUTOFIX_NEXT_WATCH_ONCE", "true")

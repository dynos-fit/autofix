...
checkout state: they skip gracefully when ``autofix/`` is missing
...
def _require_autofix() -> None:
    if not (REPO_ROOT / "autofix").is_dir():
        pytest.skip("autofix/ not present in this checkout")
...
def test_cold_build_autofix_standalone_under_30s(tmp_path: Path) -> None:
    ...
    repo's ``.autofix/`` directory — that would leak state between
    test runs and potentially confuse neighboring tests.
    """

    _require_autofix()

    # Shallow-copy autofix + autofix into a scratch dir so the real
    ...
    # under .autofix/state/index/ — without this assertion the
    # budget check would pass trivially on a cache-less build.
    manifest_path = tmp_path / ".autofix" / "state" / "index" / "manifest.json"
...
def test_symbol_record_subset_equivalence(tmp_path: Path) -> None:
    ...
    shards_root = tmp_path / ".autofix" / "state" / "index" / "shards"
...
def test_no_external_scip_python_import() -> None:
    """AC #27: no file under ``autofix/`` imports ``scip_python``
    ...
    autofix_pkg_dir = REPO_ROOT / "autofix"
    forbidden_tokens = ("scip_python", "scip_python_protobuf")

    offenders: list[tuple[str, int, str]] = []
    for py_file in autofix_pkg_dir.rglob("*.py"):
        try:

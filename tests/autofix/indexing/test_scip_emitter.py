"""Tests for ``autofix.indexing.scip_emitter`` (AC #2 / #4 / #5 / #6 / #27).
...
def test_scip_emitter_module_imports() -> None:
    """AC #2: ``autofix.indexing.scip_emitter`` is importable and
    exposes ``emit_document`` + ``SCIP_JSON_SCHEMA_VERSION``."""

    from autofix.indexing import scip_emitter
...
def test_emit_document_produces_scip_json_v1(tmp_path: Path) -> None:
    ...
    from autofix.indexing.scip_emitter import emit_document
...
def test_no_scip_python_or_protobuf_imports_in_emitter() -> None:
    ...
    emitter_path = REPO_ROOT / "autofix" / "indexing" / "scip_emitter.py"
...
def test_no_production_file_imports_scip_python_or_protobuf() -> None:
    """AC #27 contributor: grep across ``autofix/`` for any
    ...
    if not autofix_pkg_dir.is_dir():
        pytest.skip("autofix/ not present in this checkout")

    # Walk *.py under autofix/ and scan for the forbidden tokens.

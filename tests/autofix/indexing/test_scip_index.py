"""Tests for ``autofix.indexing.scip_index`` (AC #1 / #3 / #7 / #8 /
...
def test_save_creates_expected_layout(tmp_path: Path) -> None:
    ...
    ``.autofix/state/index/{manifest.json,reverse_refs.json,.lock,shards/}``."""

    _make_tiny_repo(tmp_path)
    _build_and_save(tmp_path)

    idx_root = tmp_path / ".autofix" / "state" / "index"
...
def test_manifest_has_schema_version_built_at_hashes(tmp_path: Path) -> None:
    ...
    manifest_path = (
        tmp_path / ".autofix" / "state" / "index" / "manifest.json"
    )
...
def test_manifest_save_fsyncs_parent_dir(
    ...
    idx_root = (tmp_path / ".autofix" / "state" / "index").resolve()
...
def test_reverse_refs_preserves_untouched_entries(tmp_path: Path) -> None:
    ...
    rr_path = tmp_path / ".autofix" / "state" / "index" / "reverse_refs.json"

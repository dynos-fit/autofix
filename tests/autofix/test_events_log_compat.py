"""Tests for autofix.telemetry.events_log.append_event.
...
def test_append_preserves_existing_file_prefix_bytes(tmp_path: Path) -> None:
    ...
    from autofix.telemetry.events_log import append_event
...
def test_legacy_and_new_rows_coexist(tmp_path: Path) -> None:
    """task-20260506-003: legacy autofix.runtime.dynos was deleted; the
    on-disk events.jsonl format is unchanged. Simulate a legacy row by
    constructing the same shape directly, then assert the new writer
    appends without disturbing the legacy prefix."""
    from autofix.telemetry.events_log import append_event

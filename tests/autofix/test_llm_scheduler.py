"""Tests for autofix.llm.scheduler.Scheduler.

Covers:
  AC #7  — exactly one run_prompt call per promoted candidate.
  AC #8  — run_prompt is the sole LLM seam in autofix/.
...
def test_prompt_template_content_first_then_json_packet(
    ...
    template_path = (
        REPO_ROOT / "autofix" / "llm" / "prompts" / "unused_import_review.md"
    )

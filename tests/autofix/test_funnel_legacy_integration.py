...
Patch target: `autofix.funnel.pipeline.load_legacy_findings`
...
import:
  1. autofix/migration.py is created (load_legacy_findings exists)
  2. autofix/funnel/pipeline.py imports load_legacy_findings from migration
     so the patch target `autofix.funnel.pipeline.load_legacy_findings` is valid.
"""
...
from autofix.funnel.pipeline import run_scan, ScanResult
...
def test_legacy_disabled_skips_loader(tmp_path):
    ...
    Patch target is autofix.funnel.pipeline.load_legacy_findings
    ...
    with patch(
        "autofix.funnel.pipeline.load_legacy_findings"
    ) as mock_loader:

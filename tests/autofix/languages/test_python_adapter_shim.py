...
After task-006 seg-2, ``autofix/parsing/tree_sitter.py`` is reduced
to a thin re-export shim pointing at ``autofix/languages/python.py``.
...
``autofix.languages.python`` so the file can be collected before
...
def test_parse_file_identity() -> None:
    """AC #41: ``autofix.parsing.tree_sitter.parse_file`` is the same
    function object as ``autofix.languages.python.parse_file``.
    """
    pytest.importorskip("autofix.languages.python")
    from autofix.languages import python as lang_python
    from autofix.parsing import tree_sitter as ts_mod
...
def test_python_adapter_registered() -> None:
    """AC #8: importing ``autofix.languages.python`` registers a
...
def test_python_adapter_parse_precise_returns_none() -> None:
    ...
    pytest.importorskip("autofix.languages.python")
    from autofix.languages.python import PythonAdapter

"""Tests for ``autofix.languages`` registry (task-006 AC #1..#5,
#38).
...
def _import_languages():
    ...
    return pytest.importorskip("autofix.languages")
...
def test_languages_module_exports_expected_public_names() -> None:
    ...
    assert hasattr(languages, "__all__"), (
        "autofix.languages must declare __all__"
    )
    assert expected.issubset(set(languages.__all__)), (
        f"autofix.languages.__all__ must be a superset of {expected!r}"
    )
    for name in expected:
        assert hasattr(languages, name), (
            f"autofix.languages must expose {name!r}"
        )
...
def test_all_adapters_registration_order() -> None:
    ...
    The side-effect imports at the end of ``autofix/languages/__init__.py``

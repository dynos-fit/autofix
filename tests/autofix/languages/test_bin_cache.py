"""Tests for ``autofix.languages.bin_cache`` (task-006 AC #22..#29,
#39).
...
* AC #27 — downloaded-file checksum mismatch → ``BinaryIntegrityError``
  + ``.tmp`` cleaned up.
* AC #28 — ``AUTOFIX_BIN_CACHE`` env var override is honored.
...
def _import_bin_cache():
    return pytest.importorskip("autofix.languages.bin_cache")
...
def test_ensure_binary_cache_hit_no_network(
    ...
    monkeypatch.setenv("AUTOFIX_BIN_CACHE", str(tmp_path))
...
# ---------------------------------------------------------------------------
# AC #28 — AUTOFIX_BIN_CACHE env var override
# ---------------------------------------------------------------------------
...
def test_bin_cache_env_var_override(
    ...
    """AC #28: when ``AUTOFIX_BIN_CACHE`` is set, the cache root
    resolves under that path (not ``~/.cache/autofix/bin``).
    ...
    override_root.mkdir()
    monkeypatch.setenv("AUTOFIX_BIN_CACHE", str(override_root))
    ...
    result = bc.ensure_binary(tool)
    assert result == target, (
        f"ensure_binary must respect AUTOFIX_BIN_CACHE; "

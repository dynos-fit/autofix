"""Tests for ``autofix.languages.go.GoAdapter`` (task-006 AC #15..#21,
#40).
...
def _import_go():
    return pytest.importorskip("autofix.languages.go")
...
def test_go_adapter_cache_key_format(
    ...
    shard_dir = tmp_path / ".autofix" / "state" / "index" / "scip-go"
...
def test_go_adapter_registered_by_default() -> None:
    ...
    to it."""
    pytest.importorskip("autofix.languages.go")

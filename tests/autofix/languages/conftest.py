"""Conftest for ``tests/autofix/languages/``.

Implements AC #37: tests marked ``@pytest.mark.requires_scip_binaries``
auto-skip when either

* the ``AUTOFIX_OFFLINE=1`` environment variable is set (CI / air-
  gapped mode), OR
* a probe call to ``autofix.languages.bin_cache.ensure_binary``
...
def _probe_scip_binaries_available() -> bool:
    ...
    try:
        from autofix.languages.bin_cache import (  # type: ignore[import-not-found]
...
def pytest_collection_modifyitems(config, items) -> None:  # noqa: D401
    ...
    AC #37: skip when ``AUTOFIX_OFFLINE=1`` OR when the binary
    probe fails.
    """
    offline = os.environ.get("AUTOFIX_OFFLINE") == "1"
    ...
    for item in items:
        ...
        if offline:
            item.add_marker(
                pytest.mark.skip(
                    reason="AUTOFIX_OFFLINE=1 set; scip binary tests skipped"
                )
            )
            continue

...
Covers:
  AC 10 — ``AUTOFIX_EMBEDDING_CACHE_DIR`` beats any policy override.
  AC 10 — ``policy["embedding_tier"]["cache_dir"]`` beats the scan_root
         default.
  AC 10 — The scan_root default is ``scan_root / ".cache/autofix/embedding"``.
...
def _emb():
    return pytest.importorskip("autofix.dedup.embedding")
...
def test_default_resolves_to_scan_root_dot_cache(
    ...
    ``scan_root / ".cache/autofix/embedding"``."""
    emb = _emb()
    monkeypatch.delenv("AUTOFIX_EMBEDDING_CACHE_DIR", raising=False)

    got = emb._resolve_embedding_cache_dir(tmp_path, None)
    expected = tmp_path / ".cache" / "autofix" / "embedding"
...
def test_policy_override_beats_scan_root_default(
    ...
    default_path = tmp_path / ".cache" / "autofix" / "embedding"
...
def test_env_var_beats_policy_override(
    ...
    """AC 10: ``AUTOFIX_EMBEDDING_CACHE_DIR`` wins over a policy
    ...
    monkeypatch.setenv("AUTOFIX_EMBEDDING_CACHE_DIR", str(env_cache))
...
def test_env_var_expanduser(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    emb = _emb()
    monkeypatch.setenv(
        "AUTOFIX_EMBEDDING_CACHE_DIR", "~/autofix-emb-test"
    )
    ...
    assert got.name == "autofix-emb-test"

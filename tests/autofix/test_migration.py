"""Unit tests for autofix.migration.load_legacy_findings.
...
autofix/migration.py is created by the production code segment.
"""
...
# ---------------------------------------------------------------------------
# AC 2 (post-task-20260506-003): inlined stdlib reader, no autofix.state dep
# ---------------------------------------------------------------------------

def test_reads_findings_via_stdlib_no_legacy_state_dep(tmp_path):
    """After task-20260506-003 inlined the legacy state reader, migration.py
    no longer depends on autofix.state. The contract is: read
    .autofix/state/current/findings.json directly via stdlib json.loads,
    return [] for missing/malformed."""
    # Empty directory — no findings.json; expect [].
    assert load_legacy_findings(tmp_path) == []

    # Populate with both supported top-level shapes.
    state_dir = tmp_path / ".autofix" / "state" / "current"
    state_dir.mkdir(parents=True)
    findings = [{"finding_id": "fp1", "status": "new", "category": "rule-x",
                 "description": "desc", "evidence": {"file": "ok.py", "line": 1, "symbol": "f"}}]

    (state_dir / "findings.json").write_text(json.dumps({"findings": findings}))
    out_wrapped = load_legacy_findings(tmp_path)
    assert len(out_wrapped) == 1

    (state_dir / "findings.json").write_text(json.dumps(findings))
    out_bare = load_legacy_findings(tmp_path)
    assert len(out_bare) == 1

    # Verify the migration module does not import autofix.state.
    import autofix.migration as mig_mod
    src = open(mig_mod.__file__).read()
    assert "import autofix.state" not in src
    assert "from autofix.state" not in src

"""Tests for autofix.telemetry.sarif.emit_sarif.

Covers AC #10:
  - $schema == https://json.schemastore.org/sarif-2.1.0.json
  - version == 2.1.0
  - runs[0].tool.driver.name == "autofix"
...
def _fake_finding(finding_id: str, relpath: str = "sample.py",
    ...
    lives inside autofix; we pass a plain dict to keep this test decoupled
...
def test_sarif_schema_and_driver(tmp_path: Path) -> None:
    ...
    assert doc["runs"][0]["tool"]["driver"]["name"] == "autofix"

"""Tests for autofix.evidence.builder and autofix.evidence.fingerprints.
...
def test_grep_no_other_json_dumps_in_hash_path() -> None:
    ...
    serializer for **hash inputs**. json.dumps usages under autofix/
    ...
    Any json.dumps in any other module under autofix/ is forbidden."""
    ...
    allowed = {
        ...
        # task-20260506-002 clean-slate-cli-cutover — `policy --show`
        # output formatter, not a hash input:
        pkg / "cli" / "policy_command.py",
        # task-20260506-003 seam relocation — these legacy seams produce
        # tool-call payloads, prompt arguments, and HTTP request bodies; none
        # of these flow into evidence-packet hashes:
        pkg / "agent_loop.py",
        pkg / "llm_backend.py",
        pkg / "platform.py",
...
def test_prompt_prefix_hash_formula() -> None:
    ...
    from autofix.evidence.fingerprints import (
...
def test_finding_id_formula() -> None:
    ...
    from autofix.evidence.fingerprints import compute_finding_fingerprint

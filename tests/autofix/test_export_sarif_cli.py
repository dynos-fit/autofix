"""Integration tests for the ``autofix export-sarif`` subcommand.
...
* AC 15 — ``--help`` is reachable from ``python3 -m
          autofix.cli.main export-sarif --help``.
...
from autofix.cli import export_sarif_command
...
def test_export_sarif_shape_check_failure_returns_three(
    ...
    from autofix.cli import export_sarif_command as cmd
...
def test_export_sarif_help_is_reachable_via_module() -> None:
    """AC 15: ``python3 -m autofix.cli.main export-sarif --help``
    returns exit 0 and prints the three flags."""
    repo_root = Path(__file__).resolve().parents[2]
    proc = subprocess.run(
        [sys.executable, "-m", "autofix.cli.main", "export-sarif", "--help"],
        cwd=str(repo_root),
        capture_output=True,
        text=True,
        timeout=30,

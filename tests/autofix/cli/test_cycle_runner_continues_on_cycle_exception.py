...
def test_continuous_loop_continues_on_cycle_exception(
    ...
    with patch("autofix.cli.cycle_runner._run_crawl_once_body", side_effect=_flaky_cycle), \
...
def test_keyboard_interrupt_propagates(tmp_path: Path) -> None:
    ...
    with patch("autofix.cli.cycle_runner._run_crawl_once_body", side_effect=KeyboardInterrupt), \
         patch("autofix.cli.cycle_runner._sleep", lambda _: None):
        rc = driver.run_crawl_continuously(
            root=tmp_path,
            mode="preview",
...
def test_system_exit_propagates(tmp_path: Path) -> None:
    ...
    with patch("autofix.cli.cycle_runner._run_crawl_once_body", side_effect=SystemExit(3)), \
         patch("autofix.cli.cycle_runner._sleep", lambda _: None):
...
def test_pidfile_written_and_removed(tmp_path: Path) -> None:
    ...
    with patch("autofix.cli.cycle_runner._run_crawl_once_body", side_effect=_capture_then_quit), \
         patch("autofix.cli.cycle_runner._sleep", lambda _: None):
        rc = driver.run_crawl_continuously(
            root=tmp_path,
            mode="preview",

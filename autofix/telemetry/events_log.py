"""Envelope-compatible events.jsonl writer for autofix.
...
``autofix.events.schema.NEW_EVENT_NAMES``; unknown names raise
...
def append_event(
    ...
    *,
    ...
    event_type:
        ...
        :data:`autofix.events.schema.NEW_EVENT_NAMES`. Any other value
        raises :class:`ValueError` (AC #13).
    scan_event_payload:
        Dict placed under the row's ``scan_event`` key. Typically the output
        of :meth:`autofix.events.schema.ScanEvent.to_payload`.
    now_iso:

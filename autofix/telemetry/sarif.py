"""SARIF 2.1.0 emitter for autofix scan findings.

Emits a deterministic, pretty-printed SARIF 2.1.0 document with one
``result`` per :class:`~autofix.evidence.schema.CandidateFinding`.
...
def compute_sarif_fingerprint_v1(finding: Any) -> str:
    ...
    :func:`autofix.evidence.fingerprints.canonical_json_bytes` so

  ...
  classmethod that:
    ...
    3. Cold build runs the task-003 two-pass walker:
       ...
       * **Pass 1** — parses every enumerated file with
         :func:`autofix.parsing.tree_sitter.parse_file` and collects
       ...
       * **Pass 2** — walks each file's symbol table
         (:func:`autofix.indexing.symbols.build_symbol_table`), then
...
Imports of :mod:`autofix.invalidation.import_resolver` and
:mod:`autofix.indexing.scip_index` are deferred to method bodies so
...
class CallGraph:
    ...
    def build_from_root(cls, root: Path) -> "CallGraph":
        ...
        if idx is not None and cls._all_shards_exist(idx, content_hashes):
            ...
            if graph is not None:
                ...
                from autofix.indexing.scip_index import SCIPIndex as _SI
    ...
    def _cold_build(cls, root: Path) -> "CallGraph":
        ...
        from autofix.parsing.tree_sitter import (
            TreeSitterLoadError,
            parse_file,
        )
    ...
    def _apply_incremental_refresh(
        ...
        from autofix.invalidation.planner import Invalidation
    ...
    def _all_shards_exist(
        ...
        ``<root>/.autofix/state/index/shards/<h[0:2]>/<h[2:4]>/<h>.json``
...
def _resolve_edges_v2(
    ...
    * Same-file references still resolve against the file's own symbol
    ...
    try:
        from autofix.invalidation.import_resolver import resolve
    except ImportError:

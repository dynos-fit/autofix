"""Git-based change detector for the autofix scan window.

The detector's job is to turn a repo root + a user flag into a
:class:`~autofix.events.schema.ChangeSet` of repo-relative
    ...
    *,
    ...
    try:
        ...
        raise GitUnavailableError(
            "git executable not found on PATH; install git to run autofix"
        ) from exc

...
:class:`~autofix.dedup.cluster_store.ClusterStore` can fall back to
...
* :class:`EmbeddingTier` — per-scan-root owner of the model-cache
  ...
  from env > policy > ``root / ".cache/autofix/embedding"``.
...
---------------------------------------------------
The previous module-level ``EMBEDDING_CACHE_DIR = Path("~/.cache/autofix/models/")``
...
1. Environment variable ``AUTOFIX_EMBEDDING_CACHE_DIR``.
2. ``policy["embedding_tier"]["cache_dir"]``.
3. Default ``scan_root / ".cache/autofix/embedding"``.
...
def _resolve_embedding_cache_dir(
    ...
    env_override = os.environ.get("AUTOFIX_EMBEDDING_CACHE_DIR")
    if env_override:
    ...
    return Path(scan_root) / ".cache" / "autofix" / "embedding"
...
class EmbeddingTier:
    ...
    policy:
        ...
        itself overridden by ``AUTOFIX_EMBEDDING_CACHE_DIR``.
    """

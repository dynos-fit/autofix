...
This module is the SOLE boundary between ``autofix`` and the locked
LLM seam exposed by :mod:`autofix.llm_backend`. Every other module under
``autofix/`` reaches the LLM through this scheduler; a grep test in
``tests/autofix/test_llm_scheduler.py::test_run_prompt_is_only_llm_seam``
...
7. **Promote**. Resolve the (rule_id, tier) template via
   :func:`autofix.llm.triage.resolve_template`; read once under the
...
# task-20260506-003: seams relocated into autofix/. This is now a
...
from autofix.telemetry.atomic import atomic_write_json
...
# AC 5 and by ``tests/autofix/test_llm_scheduler_decisions.py
...
class Scheduler:
    ...
    def _resolve_cache_root(policy_cache_dir: str | None, scan_root: Path) -> Path:
        ...
        to ``<scan_root>/.cache/autofix/llm_prompt_prefix``.
        """
        env_value = os.environ.get("AUTOFIX_LLM_CACHE_DIR")
        if env_value:
            return Path(env_value).expanduser()
        if policy_cache_dir:
            return Path(policy_cache_dir).expanduser()
        return scan_root / ".cache" / "autofix" / "llm_prompt_prefix"
    ...
    def _cache_put(
        ...
        *,
        ...
        install to :func:`autofix.telemetry.atomic.atomic_write_json`
    ...
    def _process_one(
        ...
        with span(
            ...
            # ``tests/autofix/test_llm_scheduler_tiered.py

...
:class:`autofix.invalidation.call_graph.CallGraph` with a cache
persisted under ``<root>/.autofix/state/index/``. The cache
consists of:
...
* The ``InvalidationComputed`` telemetry row is emitted by
  :mod:`autofix.funnel.pipeline`; we only set the flag.
"""
...
from autofix.indexing.scip_emitter import (
...
# ``.autofix/`` so we never write into the legacy ``.autofix/``
# locked path (AC #24).
INDEX_ROOT_REL: str = ".autofix/state/index"
...
class SCIPIndex:
    ...
    * :meth:`get_symbol` — O(1) lookup after load, never raises.
    ...
    def _atomic_write_json(self, final_path: Path, obj: dict) -> None:
        ...
        ``tests/autofix/indexing/test_scip_index.py`` line ~239), not
    ...
    def _shard_path_for_hash(root: Path, content_hash: str) -> Path:
        ...
        ``<root>/.autofix/state/index/shards/ab/cd/abcdef....json``.

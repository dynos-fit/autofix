...
into :class:`autofix.evidence.schema.CandidateFinding` records.
...
def _read_legacy_findings(
    root: Path,
    *,
    log: Callable[[str], None] | None,
) -> list[dict]:
    """Read the legacy findings.json shape directly from disk.

    Returns ``[]`` when the file is missing, unreadable, or contains
    malformed JSON. Tolerates two top-level shapes the legacy writer
    produced over its lifetime: a bare ``list`` and a ``{"findings": [...]}``
    wrapper.
    """
    path = root / ".autofix" / "state" / "current" / "findings.json"
    if not path.exists():
        return []
    try:
        ...
        if log is not None:
            log(f"could not load findings file: {exc}")
        return []
...
def load_legacy_findings(
    ...
    * Performs zero writes to any locked surface; the only IO is the
    ...
    """
    raw_entries = _read_legacy_findings(root, log=log)

    if not isinstance(raw_entries, list):
        # Defensive: an unexpected top-level shape collapses to empty.
        return []
    ...
    return projected
# task-20260506-003: scorched-earth rename complete

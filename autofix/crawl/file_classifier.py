...
def classify_file(
    ...
    read_head: Optional[Callable[[Path], str]] = None,
) -> FileClass:
    """Return the :class:`FileClass` for ``path``.

    Mostly pure: no filesystem I/O unless the caller supplies a
    ``read_head`` callable (see below). Never raises -- on any
    internal exception the function returns :attr:`FileClass.unknown`
    so a single pathological path cannot poison the crawler.
    ...
    ----------
    path:
        The path to classify. Need not exist on disk for the path-
        only branches.
    ...
    read_head:
        Optional callable returning the first ~8KB of ``path``'s
        content as a UTF-8 string. When provided AND the path-only
        classification would have returned :attr:`FileClass.source`,
        the head is scanned for ``DO NOT EDIT`` / ``Generated by`` /
        ``@generated`` markers via :func:`is_generated`. A match
        bumps the result to :attr:`FileClass.generated`.

        The caller owns the read budget; pass ``None`` (default) to
        skip the IO. Errors raised by ``read_head`` are swallowed;
        the result falls back to :attr:`FileClass.source`.
    """
    try:
        ...
        # 11. source -- with optional content-aware bump to generated.
        # The path-only branches above (1-10) would have caught files
        # that LOOK generated (proto stubs etc). This branch handles
        # files whose path looks normal but whose CONTENT carries a
        # generated-marker comment. Only fires when the caller opted
        # in by passing ``read_head``.
        if suffix in _SOURCE_EXTS:
            if read_head is not None:
                try:
                    head = read_head(path)
                except Exception:  # noqa: BLE001 -- read budget belongs to caller
                    head = None
                if head and is_generated(path, head):
                    return FileClass.generated

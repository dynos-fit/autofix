"""Re-export shim for ``autofix.languages.python`` (task-006 AC #9).

The tree-sitter Python parser was moved to
:mod:`autofix.languages.python` in task-006 seg-2. This module is
retained as a thin forwarding shim so every task-001..005 import path
(``from autofix.parsing.tree_sitter import parse_file``, etc.)
continues to resolve unchanged.

Why this is non-trivial
-----------------------
Existing tests drive the parser cache invalidation by monkeypatching
``_load_language`` on ``autofix.parsing.tree_sitter``. The
underlying ``_ensure_parser`` in :mod:`autofix.languages.python`
...
3. A module-class promotion to ``_ForwardingShim`` gives the shim a
   ...
   :mod:`autofix.languages.python`. That means
...
from autofix.languages import python as _py
from autofix.languages.python import (
    ParseResult,
    TreeSitterLoadError,
    parse_file,
)
...
def __getattr__(name):  # PEP 562 — lazy attribute lookup on the shim.
    ...
    try:
        ...
        raise AttributeError(
            f"module 'autofix.parsing.tree_sitter' has no attribute {name!r}"
        ) from exc

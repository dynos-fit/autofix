...
* ``canonical_json_bytes`` — the SOLE JSON-to-bytes serializer for any hash
  input anywhere under ``autofix/``. Other modules MUST NOT call
  ``json.dumps`` on hash inputs; doing so is a contract violation enforced
  by ``tests/autofix/test_evidence_builder.py::test_grep_no_other_json_dumps_in_hash_path``.
...
def canonical_json_bytes(obj: Any) -> bytes:
    ...
    * UTF-8 encoding — the bytes fed to ``hashlib.sha256`` are explicit
    ...
    This is the SOLE serializer for hash inputs under ``autofix/``.

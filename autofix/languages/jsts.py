...
* Implement :meth:`JSTSAdapter.scip_index` — invoke the
  ...
  :func:`autofix.languages.bin_cache.ensure_binary`, persist the
  output under
  ``<repo_root>/.autofix/state/index/scip-ts/<hash>.scip`` using
  the same 4-step atomic-rename + ``flock`` pattern as
  :mod:`autofix.languages.bin_cache`.
...
from autofix.languages import bin_cache, register
from autofix.languages.adapter_emission import mark_adapter_registered
...
# ``autofix/parsing/tree_sitter.py`` shim and produces a false-positive
# edge. Mirrors the workaround in autofix/languages/python.py.
...
class _EmptyParseResult:
    ...
    :class:`autofix.languages.python.ParseResult` so downstream
...
# ``autofix.funnel.pipeline._emit_invalidation_computed_event``.
# ---------------------------------------------------------------------------


# One-shot per-scan emission is tracked via the contextvar-scoped
# registry in :mod:`autofix.languages.adapter_emission` (AC 6-9).
...
def _acquire_shard_lock(lock_path: Path):
    ...
    Duplicated from :func:`autofix.languages.bin_cache._acquire_lock`
...
def _atomic_rename(tmp: Path, final: Path, state_dir: Path) -> None:
    ...
    Mirrors :func:`autofix.languages.bin_cache._atomic_install`
...
class JSTSAdapter:
    ...
    def scip_index(self, workdir: Path) -> Path | None:
        ...
        :func:`autofix.languages.bin_cache.ensure_binary`. On a
        ...
        state_dir = repo_root / ".autofix" / "state" / "index" / "scip-ts"
        try:

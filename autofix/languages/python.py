...
importable so other ``autofix`` subsystems can be introspected even
...
# ``tree_sitter`` wheel with the in-repo ``autofix/parsing/tree_sitter.py``
...
class PythonAdapter:
    ...
    :mod:`autofix.indexing.call_graph`) rather than a second
    tree-sitter parse.
    """
    ...
    def symbol_kind(self, node) -> str:
        ...
        is handled inside :mod:`autofix.indexing.symbols`. The
...
# moment ``autofix.languages`` imports this submodule. We import
...
from autofix.languages import register as _register  # noqa: E402

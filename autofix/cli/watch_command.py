"""The ``autofix watch`` subcommand.

Long-running Watchman-backed watcher for the autofix pipeline.
...
* ``AUTOFIX_WATCH_ONCE`` (truthy non-empty, not ``"0"``/``"false"``)
...
HELP_EPILOG = (
    "Test escape: set AUTOFIX_WATCH_ONCE=1 to process one "
...
def _watch_once_enabled() -> bool:
    """Return True when ``AUTOFIX_WATCH_ONCE`` is set to a truthy
    non-empty value other than ``"0"``/``"false"`` (case-insensitive).
    """
    raw = os.environ.get("AUTOFIX_WATCH_ONCE", "")
    if not raw:
        return False
...
def run(args: argparse.Namespace, *, root: Path | None = None) -> int:
    """Entry point for the `autofix watch` subcommand.
...
def _dispatch_scan(root: Path, changeset: ChangeSet) -> None:
    ...
    from autofix.funnel.pipeline import run_scan  # local import — avoid
